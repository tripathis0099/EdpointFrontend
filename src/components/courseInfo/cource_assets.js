...
export const VideosSlide = ({ videos }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#1b1a55' }}>
      ...
      {videos.map((video) => {
        return (<div style={{ display: 'flex', gap: '16px', alignItems: 'center', color: '#fff', padding: '10px', borderBottom: '1px solid #ffffff52' }} onClick={()=>{setCurrentVideo(`https://www.youtube.com/embed/${video.url.split('/')[video.url.split('/').length - 1]}`); navigate('/watchvideo', { state: { currentVideo: `https://www.youtube.com/embed/${video.url.split('/')[video.url.split('/').length - 1]}`}});
      }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#1b1a55' }}>
...

      {
        notes.map((note) => {
          ...
          return (
            <Link to={"/readNote"} style={{ display: 'flex', gap: '10px', alignItems: 'center', color: '#fff', padding: '10px', justifyContent: 'space-between', borderBottom: '1px solid #ffffff52' }}>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <h5>{note.title}</h5>
