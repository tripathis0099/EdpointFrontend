...
export const VideosSlide = ({ videos }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#1b1a55' }}>
      ...
      {videos.map((video) => {
        return (
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', color: '#fff', padding: '10px', borderBottom: '1px solid #ffffff52' }} onClick={()=>{setCurrentVideo(`https://www.youtube.com/embed/${video.url.split('/')[video.url.split('/').length - 1]}`); navigate('/watchvideo', { state: { currentVideo: `https://www.youtube.com/embed/${video.url.split('/')[video.url.split('/').length - 1]}`}});
      }}>
          <img style={{ width: '124px', height: 'auto', borderRadius: '5px' }} src={`https://img.youtube.com/vi/${video.url.split('/')[video.url.split('/').length - 1]}/default.jpg`} />
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <h5>{video.title}</h5>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#1b1a55' }}>
...

      {
        notes.map((note) => {
          ...
          return (
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', color: '#fff', padding: '10px', justifyContent: 'space-between', borderBottom: '1px solid #ffffff52' }} onClick={() => navigate2('/readNote', { state: { currentNote: note.url }})}>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <h5>{note.title}</h5>
